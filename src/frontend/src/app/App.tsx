...
// Acquire VS Code API once at module load — throws if called more than once.
const vscodeApi = (() => {
  try { return (window as any).acquireVsCodeApi?.(); }
  catch { return undefined; }
})();

// ── Bridge proxy via extension host ──────────────────────────────────────
// When running inside VS Code, all HTTP calls are routed through the extension
// host via postMessage so they work regardless of webview sandbox, IPv6, or
// remote VS Code (SSH / Codespaces). Direct fetch is used only in browser dev.

const BRIDGE_DEV = `http://localhost:${(window as any).BRIDGE_PORT || 8080}`;
const pendingCalls = new Map<string, { resolve: (d: any) => void; reject: (e: Error) => void }>();
let _callId = 0;

// Module-level listener so responses are matched even if component re-renders.
window.addEventListener('message', (event: MessageEvent) => {
  const msg = event.data;
  if (msg?.type !== 'bridge-response') return;
...
async function bridgeCall(method: string, path: string, body?: object): Promise<any> {
  if (vscodeApi) {
    const id = String(++_callId);
    return new Promise((resolve, reject) => {
      pendingCalls.set(id, { resolve, reject });
      setTimeout(() => {
        if (pendingCalls.delete(id)) reject(new Error('Request timed out'));
      }, 300_000); // 5 min — long enough for full-repo indexing
      vscodeApi.postMessage({ type: 'bridge-call', id, method, path, body });
    });
  }
  // Browser dev-mode fallback
  const res = await fetch(`${BRIDGE_DEV}${path}`, {
    method,
...
export default function App() {
  ...
  // UI-only messages from the extension (errors, reindex notifications, fix results)
  useEffect(() => {
    const handler = (event: MessageEvent) => {
      const msg = event.data;
      if (!msg?.type) return;
      if      (msg.type === 'error-captured')  setCapturedError(msg.data as string);
      else if (msg.type === 'reindex-start')   { setReindexingFile(msg.file as string); setTimeout(() => setReindexingFile(''), 3000); }
      else if (msg.type === 'fix-result') {
        const { applied, total } = msg as { applied: number; total: number };
        setFixState(applied > 0 ? 'done' : 'error');
        setFixSummary(applied === total ? `${applied} edit${applied !== 1 ? 's' : ''} applied` : `${applied}/${total} edits applied`);
      }
    };
  ...
  useEffect(() => {
    bridgeCall('GET', '/status')
      .then(data => {
        if (data.indexed_chunks > 0) { setIndexedChunks(data.indexed_chunks); setIndexState('done'); }
        else if (WORKSPACE) handleIndex(WORKSPACE);
      })
      .catch(() => { if (WORKSPACE) setTimeout(() => handleIndex(WORKSPACE), 3000); });
  }, []);
  ...
  const handleIndex = async (repoPath: string = WORKSPACE) => {
    ...
    setIndexError('');
    try {
      const data = await bridgeCall('POST', '/index', { repo_path: repoPath });
  ...
  const handleAnalyze = async () => {
    ...
    setFixSummary('');
    try {
      const data = await bridgeCall('POST', '/analyze', { error_log: errorLog });
  ...
  const indexBadge = () => {
    if (reindexingFile)     return <span className="flex items-center gap-1 text-[10px] text-[#cca700]"><RefreshCw className="w-3 h-3 animate-spin" />{reindexingFile}</span>;
    if (indexState === 'indexing') return <span className="flex items-center gap-1 text-[10px] text-[#cca700]"><Loader className="w-3 h-3 animate-spin" />Indexing...</span>;
    if (indexState === 'done')     return <span className="flex items-center gap-1 text-[10px] text-[#4ec9b0]"><CheckCircle className="w-3 h-3" />{indexedChunks} functions indexed</span>;
    if (indexState === 'error')    return <span className="text-[10px] text-[#f44747]" title={indexError}>Index error — retry</span>;
    return null;
  };
    ...
    <div className="flex items-center gap-2">
      ...
      {capturedError && (
        <div className="mx-4 mt-3 px-3 py-2 bg-[#2d1a1a] border border-[#6b2d2d] rounded flex items-center gap-2">
          ...
          <button onClick={() => { setErrorLog(capturedError); setCapturedError(''); }}
      ...
      <div className="px-4 py-4 border-b border-[#2d2d2d]">
        ...
        <textarea value={errorLog} onChange={e => setErrorLog(e.target.value)}
        ...
        <div className="mt-3 flex gap-2">
          ...
          {analysisResult && vscodeApi && (
            <button onClick={handleApplyFix} disabled={fixState === 'generating' || fixState === 'applying'}
